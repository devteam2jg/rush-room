...
import { mediasoupConfig } from '~/src/mediasoup-temp/mediasoup.config';
...
export class MediasoupService implements OnModuleInit, OnModuleDestroy {
  ...
  private async initializeWorkers() {
    ...
    for (let i = 0; i < numWorkers; i++) {
      ...
      worker.on('died', () => {
        this.logger.error(
          'mediasoup-temp worker died, exiting in 2 seconds... [pid:%d]',
          worker.pid,
        );
  ...
  async createWebRtcTransport(
    ...
    this.logger.log(`transport options ${options2.preferUdp}`);
    // SCTP 설정이 필요한 경우
